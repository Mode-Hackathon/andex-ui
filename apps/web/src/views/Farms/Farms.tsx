...
const Farms: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const stakedOnlyFarms = useMemo(
    () => getStakedMinProgramFarms(activeFarms),
    [activeFarms]
  );

  const stakedInactiveFarms = useMemo(
    () => getStakedMinProgramFarms(inactiveFarms),
    [inactiveFarms]
  );

  const stakedArchivedFarms = useMemo(
    () => getStakedMinProgramFarms(archivedFarms),
    [archivedFarms]
  );
  ...
  const chosenFarms = useMemo(() => {
    ...
    if (boostedOnly || stableSwapOnly) {
      const boostedOrStableSwapFarms = chosenFs.filter(
        (farm) =>
          (boostedOnly && farm.boosted) || (stableSwapOnly && farm.isStable)
      );

      const stakedBoostedOrStableSwapFarms = getStakedMinProgramFarms(
        boostedOrStableSwapFarms
      );

      chosenFs = stakedOnly
        ? farmsList(stakedBoostedOrStableSwapFarms)
        : farmsList(boostedOrStableSwapFarms);
    }
      ...
      switch (sortOption) {
        case "apr":
          return orderBy(
            ...
            (farm: FarmWithStakedValue) =>
              Number(farm.apr) + Number(farm.lpRewardsApr),
            "desc"
          );
        case "multiplier":
          return orderBy(
        ...
        case "latest":
          return orderBy(
            orderBy(
              farms,
              (farm: FarmWithStakedValue) => Number(farm.pid),
              "desc"
            ),
            ["version"],
            "desc"
  ...
  return (
    <FarmsContext.Provider value={providerValue}>
      ...
      <Page>
        ...
        {isInactive && chainId === ChainId.MODE_MAINNET && (
        ...
        {viewMode === ViewMode.TABLE ? (
          <Table
            farms={chosenFarmsMemoized as V2Farm[]}
            cakePrice={cakePrice}
