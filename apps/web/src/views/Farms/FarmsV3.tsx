...
export interface V3FarmWithoutStakedValue
  extends FarmV3DataWithPriceAndUserInfo {
  version: 3;
}
...
const Farms: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const normalizedUrlSearch = useMemo(
    () => (typeof urlQuery?.search === "string" ? urlQuery.search : ""),
    [urlQuery]
  );
  ...
  const userDataReady =
    !account ||
    (!!account &&
      (chainId && supportedChainIdV2.includes(chainId)
        ? v2UserDataLoaded
        : true) &&
      (chainId && supportedChainIdV3.includes(chainId)
        ? v3UserDataLoaded
        : true));
  ...
  const activeFarms = farmsLP.filter(
    (farm) =>
      ...
      (farm.version === 3
        ...
        : !v2PoolLength || v2PoolLength > farm.pid)
  );
  ...
  const stakedOnlyFarms = useMemo(
    () => getStakedFarms(activeFarms),
    [activeFarms]
  );

  const stakedInactiveFarms = useMemo(
    () => getStakedFarms(inactiveFarms),
    [inactiveFarms]
  );

  const stakedArchivedFarms = useMemo(
    () => getStakedFarms(archivedFarms),
    [archivedFarms]
  );
  ...
  const farmsList = useCallback(
    (farmsToDisplay: V2AndV3Farms): V2StakeValueAndV3Farm[] => {
      const farmsToDisplayWithAPR: any = farmsToDisplay.map((farm) => {
        ...
        const totalLiquidityFromLp = new BigNumber(
          farm?.lpTotalInQuoteToken ?? 0
        ).times(farm.quoteTokenPriceBusd);
        // Mock 1$ tvl if the farm doesn't have lp staked
        const totalLiquidity =
          totalLiquidityFromLp.eq(BIG_ZERO) && mockApr
            ? BIG_ONE
  ...
  const chosenFarms = useMemo(() => {
    ...
    if (v3FarmOnly || v2FarmOnly || boostedOnly || stableSwapOnly) {
      ...
      chosenFs = stakedOnly
        ? farmsList(stakedFilterFarmsWithTypes)
        : farmsList(filterFarmsWithTypes);
    }
      ...
      switch (sortOption) {
        case "apr":
          return orderBy(
            farms,
            (farm) =>
              farm.version === 3 ? Number(farm.cakeApr) : farm.apr ?? 0,
            "desc"
          );
        case "multiplier":
          return orderBy(
