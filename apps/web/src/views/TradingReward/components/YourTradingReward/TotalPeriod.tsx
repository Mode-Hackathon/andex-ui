...
const TotalPeriod: React.FC<React.PropsWithChildren<TotalPeriodProps>> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t("Claim your rewards before expiring."),
    {
      placement: "bottom",
    }
  );
  ...
  const notReadyForClaim = useMemo(() => {
    ...
    return totalAvailableClaimData.filter((campaign) => {
      const campaignIncentive = campaignIdsIncentive.find(
        (incentive) =>
          incentive.campaignId.toLowerCase() ===
  ...
  const timeRemaining =
    rewardExpiredSoonData?.campaignClaimEndTime - currentDate;
    ...
    return unclaimData
      .map((available) => {
        ...
        if (currentReward) {
          ...
          return reward.div(rewardCakePrice).isNaN()
            ? 0
            : reward.div(rewardCakePrice).toNumber();
        }
        return 0;
      })
      .reduce((a, b) => new BigNumber(a).plus(b).toNumber(), 0);
  ...
  const expiredUSDPrice = useMemo(
    () => getUSDValue(rewardExpiredSoonData),
    [getUSDValue, rewardExpiredSoonData]
  );
              ...
              <Message variant="warning" mt="16px">
                <MessageText>
                  <Text m="0 4px" as="span">
                    {t(
                      "Please switch your network to BNB Chain to claim your trading rewards."
                    )}
                  </Text>
                </MessageText>
              </Message>
              ...
              <Message variant="primary" mt="16px">
                <MessageText>
                  <TooltipText bold as="span">
                    {minAmountDisplay({
                      amount: notReadyForClaimUSDPrice,
                      prefix: "$",
                    })}
                  </TooltipText>
                  <Text m="0 4px" as="span">
                    {t(
                      "from the recent campaign period is under tallying and will be available for claiming soon."
                    )}
                  </Text>
                </MessageText>
              </Message>
