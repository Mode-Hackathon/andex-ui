...
import {
  useTradingRewardContract,
  useTradingRewardTopTraderContract,
...
export interface AllTradingRewardPairDetail {
  ...
  campaignPairs: {
    [campaignId in string]: { [chainId in string]: Array<string> };
  };
...
const fetchCampaignPairs = async (
  campaignIds: Array<string>,
  type: RewardType
) => {
  const newData: {
  ...
  await Promise.all(
    campaignIds.map(async (campaignId: string) => {
      const pair = await fetch(
        `${TRADING_REWARD_API}/campaign/pair/campaignId/${campaignId}/type/${type}`
      );
...
const fetchCampaignIdsIncentive = async (
  ...
  const campaignIdsIncentive: Incentives[] = await Promise.all(
    campaignIds.map(async (campaignId: string) => {
      const incentives = await tradingRewardContract.read.incentives([
        campaignId,
      ]);
      const [
      ...
      const formatted = {
        ...
        campaignClaimTime: new BigNumber(
          campaignClaimTime.toString()
        ).toNumber(),
        campaignClaimEndTime: new BigNumber(
          campaignClaimEndTime.toString()
        ).toNumber(),
        totalRewardUnclaimed: new BigNumber(
          totalRewardUnclaimed.toString()
        ).toJSON(),
      };
      return {
        campaignId,
...
const fetUserQualification = async (
  tradingRewardContract: ReturnType<typeof getTradingRewardContract>
) => {
  ...
  await Promise.all(
    campaignIds.map(async (campaignId: string) => {
      const reward = await fetch(
        `${TRADING_REWARD_API}/reward/campaignId/${campaignId}/type/${type}`
      );
...
const useAllTradingRewardPair = ({
  status,
  type,
}: UseAllTradingRewardPairProps): AllTradingRewardPair => {
  const tradingRewardContract = useTradingRewardContract({
    chainId: ChainId.MODE_MAINNET,
  });
  const tradingRewardTopTradersContract = useTradingRewardTopTraderContract({
    chainId: ChainId.MODE_MAINNET,
  });
  const contract =
    type === RewardType.CAKE_STAKERS
      ? tradingRewardContract
      : tradingRewardTopTradersContract;
    ...
    async () => {
      try {
        const campaignsResponse = await fetch(
          `${TRADING_REWARD_API}/campaign/status/${status}/type/${type}`
        );
        const campaignsResult = await campaignsResponse.json();
        const campaignIds: Array<string> = campaignsResult.data;

        const [campaignPairs, campaignIdsIncentive, qualification, rewardInfo] =
          await Promise.all([
