...
import {
  ...
  CampaignIdInfoDetail,
} from "views/TradingReward/hooks/useCampaignIdInfo";
...
export interface UserCampaignInfoDetail
  extends UserCampaignInfoResponse,
    CampaignIdInfoDetail {
  campaignId: string;
  canClaim: string;
...
const useAllUserCampaignInfo = ({
  campaignIds,
  type,
}: UseAllUserCampaignInfoProps): AllUserCampaignInfo => {
  const { address: account } = useAccount();
  const tradingRewardAddress =
    type === RewardType.CAKE_STAKERS
      ? getTradingRewardAddress(ChainId.MODE_MAINNET)
      : getTradingRewardTopTradesAddress(ChainId.MODE_MAINNET);
      ...
      try {
        const allUserCampaignInfo = await Promise.all(
          campaignIds.map(async (campaignId: string) => {
            const [userCampaignInfoResponse, userInfoQualificationResponse] =
              await Promise.all([
                fetch(
            ...
            const [userCampaignInfoResult, userInfoQualificationResult] =
              await Promise.all([
                userCampaignInfoResponse.json(),
                userInfoQualificationResponse.json(),
              ]);

            const userCampaignInfo: CampaignIdInfoResponse =
              userCampaignInfoResult.data;
                ...
                (i) =>
                  ({
                    ...
                    args: [
                      ...
                      account,
                      BigInt(
                        new BigNumber(Number(i.tradingFee).toFixed(8))
                          .times(1e18)
                          .toString()
                      ),
                    ] as const,
